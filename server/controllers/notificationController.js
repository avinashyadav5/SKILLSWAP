...
// ✅ Fixed: helper to safely get io from app.locals
function getIO() {
  return global._skillswap_io || null;
}
...
exports.createNotification = async (req, res) => {
  ...
  try {
    ...
    const io = getIO();
    if (io && !mutedUsers.has(String(userId))) {
      io.to(userId.toString()).emit("notification", note);
    }
...
// ✅ Set IO reference (called from server.js)
exports.setIO = (io) => { global._skillswap_io = io; };
