...
const { apiLimiter, geminiLimiter, authLimiter } = require('./middleware/rateLimiter'); // ✅ Added
...
app.use('/api/chatbot', geminiLimiter, chatbotRoute);         // ✅ Rate limited
app.use('/api/learning-path', learningPathRoutes);
app.use('/api/validate-subject', geminiLimiter, validateSubjectRoutes); // ✅ Rate limited
app.use('/api/recommendations', geminiLimiter, recommendationRoutes);   // ✅ Rate limited
...
app.use('/api/progress', skillProgressRoutes);
app.use('/api/stats', statsRoute);
app.use('/api/study-buddy', geminiLimiter, studyBuddyRoutes); // ✅ Rate limited
...
// ✅ Fixed: Use app.locals instead of global.io to avoid global namespace pollution
app.locals.io = io;
...
io.on('connection', (socket) => {
  ...
  // --- Group Video Room Invites ---
  socket.on('video_room_invite', ({ to, from, fromName, url }) => {
    try {
      console.log(`📹 video_room_invite ${from} -> ${to}`);
      io.to(to.toString()).emit('video_room_invite', { from, fromName, url });
    } catch (err) {
      console.error('video_room_invite error:', err);
    }
  });
